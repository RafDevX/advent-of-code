...
void mark_line_in_map(vector<vector<int>> line, vector<vector<int>> *map)
{
	if (line[0][0] == line[1][0]) {
		...
		/*
	} else if (line[0][0] == line[0][1] && line[1][0] == line[1][1]) {
		for (int i = min(line[0][0], line[1][1]); i <= max(line[0][0], line[1][1]); i++) {
			(*map)[i][i]++;
		}
	} else if (line[0][0] == line[1][1] && line[0][1] == line[1][0]) {
		for (int i = 0; i <= (max(line[0][0], line[0][1]) - min(line[0][0], line[0][1])); i++) {
			cout << "d-point (" << min(line[0][0], line[0][1]) + i << ", " << max(line[0][0], line[0][1]) - i << ")\n";
			(*map)[min(line[0][0], line[0][1]) + i][max(line[0][0], line[0][1]) - i]++;
		}
			*/
